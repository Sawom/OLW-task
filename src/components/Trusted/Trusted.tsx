        ...
        <div className="relative overflow-hidden w-full">
          <div className="flex bg-[#F6F6F6] py-4 w-max items-center animate-infinite-scroll gap-12 md:gap-20 lg:gap-24">
            ...
            {brandLogos.map((logo, index) => (
              <div key={`set1-${index}`} className="flex-shrink-0 w-[120px] h-[60px]  relative  transition-all duration-300 opacity-100 cursor-pointer">
                 <Image 
                   src={logo} 
                   fill 
            ...
            {brandLogos.map((logo, index) => (
              <div key={`set2-${index}`} className="flex-shrink-0 w-[120px] h-[60px]  relative transition-all duration-300 opacity-100 cursor-pointer">
                 <Image 
                   src={logo} 
                   fill 
