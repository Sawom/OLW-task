    ...
    <section className="py-12 bg-white">
      <div className="container mx-auto px-4 md:px-8 lg:px-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 mt-2">Choose a plan that's right for you</p>
        </div>

        {/* Toggle Switch */}
        <div className="flex items-center justify-center gap-4 mb-16 relative">
          <span className={`text-[16px] font-medium ${!isYearly ? 'text-black' : 'text-gray-400'}`}>Pay Monthly</span>
          
          <button 
            onClick={() => setIsYearly(!isYearly)}
            className="w-14 h-7 bg-gray-300 rounded-full relative transition-colors duration-300 focus:outline-none"
          >
            <div className={`absolute top-1 left-1 w-5 h-5 bg-white rounded-full transition-transform duration-300 ${isYearly ? 'translate-x-7' : 'translate-x-0'}`} />
          </button>

          <span className={`text-[16px] font-medium ${isYearly ? 'text-black' : 'text-gray-400'}`}>Pay Yearly</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
          {plans.map((plan, index) => (
            ...
            >
              ...
              <p className={`text-[14px] leading-relaxed mb-8 text-gray-500 group-hover:text-pink-100`}>
                {plan.description}
              </p>
              
              <div className="flex items-baseline gap-1 mb-8">
                <span className="text-[48px] font-bold">${plan.price}</span>
                <span className={`text-[16px] text-gray-400 group-hover:text-pink-100`}>
                  / Month
                </span>
              </div>

              <button className={`w-full py-4 rounded-xl font-bold transition-all mb-10
                ...
                {plan.features.map((feature, fIndex) => (
                  <div key={fIndex} className="flex items-start gap-3">
                    <div className={`mt-1 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center
                      ...
                      {feature.included ? <Check size={14} strokeWidth={3} /> : <X className='group-hover:bg-white group-hover:text-black group-hover:rounded-full ' strokeWidth={2} size={14} />}
                    </div>
                    <span className={`text-[14px]  ${!feature.included ? 'text-gray-400' : ''} group-hover:text-white `}>
                      {feature.text}
                    </span>
                  </div>
                ))}
